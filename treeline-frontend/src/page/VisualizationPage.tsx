...
const VisualizationPage: React.FC = () => {
  ...
  const groupByModule = () => {
    const moduleGroups: {[key: string]: {x: number, y: number, children: string[]}} = {};
    
    data.nodes.forEach(node => {
      if (node.type === 'module') {
        moduleGroups[node.id] = { x: 0, y: 0, children: [] };
      }
    });
    ...
    data.links.forEach(link => {
      if (link.type === 'contains') {
        ...
        const module = moduleGroups[sourceId];
        if (module) {
          module.children.push(targetId);
        }
      }
    });
  ...
  const forceCluster = () => {
    ...
    function force(alpha: number) {
      ...
      nodes.forEach(node => {
        const group = Object.values(moduleGroups).find(g => g.children.includes(node.id));
        if (group) {
  ...
  useEffect(() => {
    ...
    const getNodeSize = (node: CodeNode) => {
      if (node.type === 'module') return 12;
      if (node.type === 'class') return 10;
      return 8;
    };
  
    const getNodeFillColor = (node: CodeNode) => {
    ...
    const getNodeStrokeColor = (node: CodeNode) => {
      if (node.type === 'module') return '#0284c7';
      if (node.type === 'class') return '#0891b2';
      return '#0d9488';
    };
    
    const getCategory = (smell: string | CodeSmell) => {
    ...
    nodeSelection.on('mouseover', function(event, d) {
      ...
      let tooltipContent = `<strong>${d.name}</strong> <span style="color: #666;">(${d.type})</span><br>`;
      ...
      if (hasQualityIssues || hasComplexity || hasSqlInjection || hasSecurityIssues || hasStyleIssues) {
        ...
        if (hasSecurityIssues) {
          ...
          tooltipContent += `• <span style="color: #b91c1c;">${count} security vulnerability${count === 1 ? '' : 'ies'} detected!</span><br>`;
        }
        
        if (hasSqlInjection) {
          const count = d.code_smells?.filter(smell => getCategory(smell) === 'sql_injection').length || 0;
          tooltipContent += `• <span style="color: #b91c1c;">${count} potential SQL injection risk${count === 1 ? '' : 's'}!</span><br>`;
        }
        ...
        if (hasStyleIssues) {
          ...
          tooltipContent += `• <span style="color: #0369a1;">${count} style issue${count === 1 ? '' : 's'}</span><br>`;
        }
        ...
        if (otherIssues.length > 0) {
          tooltipContent += `• <span style="color: #b91c1c;">${otherIssues.length} other code quality issues</span><br>`;
        }
      ...
      if (d.docstring) {
        tooltipContent += `<div style="color: #059669; margin: 5px 0;"><em>${d.docstring}</em></div>`;
      }
      ...
      if (d.metrics) {
        ...
        if (d.metrics.lines) {
          tooltipContent += `• Lines: ${d.metrics.lines}<br>`;
        }
        if (d.metrics.functions) {
          tooltipContent += `• Functions: ${d.metrics.functions}<br>`;
        }
        if (d.metrics.classes) {
          tooltipContent += `• Classes: ${d.metrics.classes}<br>`;
        }
      }
    ...
    if (collisionForce) {
      collisionForce.radius((d: any) => {
        const textWidth = (d as CodeNode).name.length * 5;
        return Math.max(getNodeSize(d as CodeNode) + textWidth / 2, 40);
      }).strength(1);
    }
