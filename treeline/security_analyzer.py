...
class TreelineSecurity:
    def __init__(self):
        ...
        self.RISKY_PATTERNS = {
            ...
            'command_injection': {
                'patterns': {
                    'system', 'popen', 'run', 'shell', 'spawn', 'call'
                },
            ...
            'file_operations': {
                'patterns': {
                    'open', 'read', 'write', 'save', 'load', 'exec', 'eval', 'remove', 'unlink'
                }
            }
        }
    ...
    def _check_all_dangerous_calls(self, node: ast.Call, filename: str) -> None:
        ...
        if isinstance(node.func, ast.Name):
            ...
            if func_name in self.RISKY_PATTERNS['command_injection']['patterns']:
                ...
                self._add_issue('command_injection',
            ...
            if isinstance(node.func.value, ast.Name):
                ...
                if orig_module in self.RISKY_PATTERNS['deserialization']['risky_modules']:
                    ...
                    if func_name in self.RISKY_PATTERNS['deserialization']['patterns']:
                        ...
                        self._add_issue('deserialization',
                ...
                if func_name in self.RISKY_PATTERNS['file_operations']['patterns']:
                    ...
                    for arg in node.args:
                        if isinstance(arg, ast.BinOp):
                            print(f"Found string concatenation in argument of {func_name}")
                            self._add_issue('file_operations',
    ...
    def _check_string_concat(self, node: ast.BinOp, filename: str) -> None:
        ...
        if ((isinstance(node.left, ast.Constant) and isinstance(node.left.value, str)) or
            ...
            if any(keyword in str(node.left.value).lower() for keyword in sql_keywords if hasattr(node.left, 'value')):
    ...
    def _check_hardcoded_secrets(self, node: ast.Assign, filename: str) -> None:
        ...
        for target in node.targets:
            if isinstance(target, ast.Name):
                ...
                if any(word in name for word in ['password', 'secret', 'key', 'token', 'api']):
                    print(f"Found secret-like name: {name}")
                    if isinstance(node.value, ast.Constant):
                        print(f"Found hardcoded value: {node.value.value}")
                        self._add_issue('hardcoded_secret',
                            f"Hardcoded secret found in variable '{target.id}'",
                            filename, node.lineno)

    def _add_issue(self, category: str, description: str, filename: str, line: int) -> None:
