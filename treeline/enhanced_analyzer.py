...
class EnhancedCodeAnalyzer:
    ...
    def _calculate_maintainability_index(self, node: ast.AST, content: str) -> float:
        ...
        operators = len([n for n in ast.walk(node) if isinstance(n, (ast.operator, ast.BoolOp, ast.Compare))])
        operands = len([n for n in ast.walk(node) if isinstance(n, (ast.Name, ast.Num, ast.Str))])
        hv = (operators + operands) * math.log(len(set([operators, operands])), 2) if operators + operands > 0 else 1
    ...
    def _calculate_cognitive_load(self, node: ast.FunctionDef) -> int:
        ...
        """
        control_structures = sum(1 for n in ast.walk(node) 
    ...
    def _analyze_file_metrics(self, content: str, file_path: Path) -> None:
        ...
        if duplication['duplicated_blocks'] > self.QUALITY_METRICS['MAX_DUPLICATED_BLOCKS']:
            self._add_issue(
                'duplication',
    ...
    def _check_class_metrics(self, class_data: Dict) -> None:
        ...
        if metrics.get('public_methods', 0) < self.QUALITY_METRICS['MIN_PUBLIC_METHODS']:
            smells.append(f"Too few public methods (< {self.QUALITY_METRICS['MIN_PUBLIC_METHODS']}, SOLID-ISP)")
    
        if metrics.get('inheritance_depth', 0) > self.QUALITY_METRICS['MAX_INHERITANCE_DEPTH']:
            smells.append(f"Deep inheritance (> {self.QUALITY_METRICS['MAX_INHERITANCE_DEPTH']} levels)")
        
        imports = metrics.get('imports', {})
        if imports.get('import_count', 0) > self.QUALITY_METRICS['MAX_IMPORT_STATEMENTS']:
            smells.append(f"Too many imports (> {self.QUALITY_METRICS['MAX_IMPORT_STATEMENTS']})")
        
        if imports.get('dependencies', 0) > self.QUALITY_METRICS['MAX_MODULE_DEPENDENCIES']:
            smells.append(f"Too many dependencies (> {self.QUALITY_METRICS['MAX_MODULE_DEPENDENCIES']})")
        ...
        for item in structure:
            ...
            if metrics:
                if metrics.get('complexity', 0) > self.QUALITY_METRICS['MAX_CYCLOMATIC_COMPLEXITY']:
                    lines.append(f"{child_indent}└─ ⚠️ High complexity ({metrics['complexity']})")
                if metrics.get('lines', 0) > self.QUALITY_METRICS['MAX_FUNCTION_LINES']:
                    lines.append(f"{child_indent}└─ ⚠️ Too long ({metrics['lines']} lines)")
                if metrics.get('nested_depth', 0) > self.QUALITY_METRICS['MAX_NESTED_DEPTH']:
                    lines.append(f"{child_indent}└─ ⚠️ Deep nesting (depth {metrics['nested_depth']})")
    ...
    def _analyze_code_duplication(self, content: str) -> Dict[str, int]:
        ...
        for i in range(len(lines)):
            for j in range(i + 1, len(lines)):
                ...
                while (i + consecutive_matches < len(lines) and 
                    j + consecutive_matches < len(lines) and 
                    lines[i + consecutive_matches].strip() == lines[j + consecutive_matches].strip()):
