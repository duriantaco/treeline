...
def format_structure(self, structure: List[Dict], indent: str = "") -> List[str]:
    ...
    for item in structure:
        ...
        if validated_item.metrics:
            if validated_item.metrics.get('complexity', 0) > self.QUALITY_METRICS['MAX_CYCLOMATIC_COMPLEXITY']:
                lines.append(f"{indent}  └─ ⚠️ High complexity ({validated_item.metrics['complexity']})")
                
            if validated_item.metrics.get('lines', 0) > self.QUALITY_METRICS['MAX_FUNCTION_LINES']:
                lines.append(f"{indent}  └─ ⚠️ Too long ({validated_item.metrics['lines']} lines)")
                
            if validated_item.metrics.get('nested_depth', 0) > self.QUALITY_METRICS['MAX_NESTED_DEPTH']:
                lines.append(f"{indent}  └─ ⚠️ Deep nesting (depth {validated_item.metrics['nested_depth']})")
